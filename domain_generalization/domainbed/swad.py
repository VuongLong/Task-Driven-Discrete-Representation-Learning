...
class LossValley1(SWADBase):
    ...
    def update_and_evaluate(self, segment_swa, val_acc, val_loss, prt_fn=None):
        ...
        print(self.threshold, self.converge_step)
        print([model.end_loss for model in self.converge_Q])
        print([model.end_step for model in self.converge_Q])
        print([model.end_step for model in self.smooth_Q])

        if not self.is_converged  or val_acc < self.converge_val:
            if len(self.converge_Q) < self.n_converge:
                return
            ...
            if min_idx == 0:
                # import pdb; pdb.set_trace()
                self.converge_val = self.converge_Q[0].end_loss
