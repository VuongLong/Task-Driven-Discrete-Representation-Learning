...
from .phinet import PhiNet, VectorQuantizedPhiNet
...
class FeatureNet(Network):
    def __init__(
        ...
        self.quantize_coef = quantize_coef
        if coefs is not None:
            for k, v in coefs.items():
                self.coefs[k] = v

        if use_quantize:
            self.phi = VectorQuantizedPhiNet(
        ...
        self.inv_discriminator = InvDiscriminator(
            n_actions=n_actions,
            n_latent_dims=n_latent_dims,
            n_units_per_layer=n_units_per_layer,
            n_hidden_layers=n_hidden_layers,
    ...
    def oracle_loss(self, z0, z1, d):
        ...
        dz = torch.cat(
            [
                ...
                torch.norm(z1.flip(0) - z0, dim=-1, p=2),
            ],
            dim=0,
        )
    ...
    def train_batch(self, x0, x1, a, d):
        ...
        z0 = self.phi(x0)
        loss = 0
        # TODO: This is questionable, do we even need to compute the loss of two consecutive latent losses?
        if getattr(self.phi, "latent_loss", None):
            loss = self.phi.latent_loss
        z1 = self.phi(x1)
        # if getattr(self.phi, "latent_loss", None):
        #     loss = loss + self.phi.latent_loss
        # z1_hat = self.fwd_model(z0, a)
        loss = self.quantize_coef * loss + self.compute_loss(z0, z1, a, d)
        loss.backward()
        self.optimizer.step()
        return loss
