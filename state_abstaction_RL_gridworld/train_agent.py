...
from models.phinet import PhiNet, VectorQuantizedPhiNet
...
if args.no_phi:
    ...
    if args.phi_path:
        modelfile = "results/models/{}/phi-{}_latest.pytorch".format(
...
if args.video:
    ...
    def plot_value_function(ax):
        s = np.asarray(
            [[np.asarray([x, y]) for x in range(args.cols)] for y in range(args.rows)]
        )
        v = np.asarray(agent.q_values(s).detach().numpy()).max(-1)
        xy = (
            OffsetSensor(offset=(0.5, 0.5)).observe(s).reshape(args.cols, args.rows, -1)
        )
        ax.contourf(
            np.arange(0.5, args.cols + 0.5),
