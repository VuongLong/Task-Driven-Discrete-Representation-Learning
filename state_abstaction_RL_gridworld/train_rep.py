...
parser.add_argument('--quantize', action='store_true', default=False,
                    help="Use Vector quantized network for the abstraction model.")
parser.add_argument('--use_vqwae', action='store_true', default=False,
                    help="Use Wasserstein Vector quantized network for the abstraction model.")
parser.add_argument('--use_sqvae', action='store_true', default=False,
                    help="Use SQVAE quantized network for the abstraction model.")
parser.add_argument('--quantize_coef', type=float, default=100,
                    help='quantize_coef')
...
assert not (args.use_vqwae and args.use_sqvae)
...
for t in range(n_samples):
    ...
    s, _, _ = env.step(a)
    st.append(copy.deepcopy(states[-1]))
    next_st.append(s)
    states.append(s)
    actions.append(a)

    # if args.walls == "empty":  # random walk
    #     continue
    # random reset
    env.reset_agent()
    states.pop(-1)
...
st = np.asarray(st)
next_st = np.asarray(next_st)

s0_ = np.asarray(states[:-1, :])

s0 = st
assert s0_.shape == s0.shape

c0 = s0[:, 0] * env._cols + s0[:, 1]
s1_ = np.asarray(states[1:, :])
s1 = np.asarray(next_st)
assert s1.shape == s1_.shape
...
# % ------------------ Setup experiment ------------------
n_updates_per_frame = min(100, args.n_updates)
n_frames = args.n_updates // n_updates_per_frame

batch_size = min(args.batch_size, args.n_samples // 2)
...
if args.type == "markov":
    fnet = FeatureNet(
        ...
        use_vqwae=args.use_vqwae,
        use_sqvae=args.use_sqvae,
        quantize_coef=args.quantize_coef,
    )
...
if args.video:
    if not args.cleanvis:
        repvis = RepVisualization(
            env, obs, batch_size=n_test_samples, n_dims=2, colors=test_c, cmap=cmap
        )
    else:
        repvis = CleanVisualization(
            env, obs, batch_size=n_test_samples, n_dims=2, colors=test_c, cmap=cmap
        )
...
get_next_batch = lambda: get_batch(
    x0[: n_samples // 2, :], x1[: n_samples // 2, :], a[: n_samples // 2]
)


def test_rep(fnet, step, hist=None):
    with torch.no_grad():
        fnet.eval()
        if args.type == "markov":
            z0 = fnet.phi(test_x0)
        ...
        # if args.quantize and hist:
        #     z0_from_encoder = fnet.phi.phi(test_x0)
        #     _, _, min_encoding_indices, _ = fnet.phi.quantizer._inference(
        #         z0_from_encoder,
        #         fnet.phi.codebook,
        #     )
        #     hist.update(min_encoding_indices)
...
# % ------------------ Measure codebook histogram ------------------
# from models.hist_codebook import HistogramCodebook

# hist = HistogramCodebook(codebook_size=args.codebook_size)
...
for frame_idx in tqdm(range(n_frames + 1)):
    for _ in range(n_updates_per_frame):
        ...
        tdist = torch.cat(
            [
                torch.as_tensor(oracle.pairwise_distances(idx, s0, s1))
                .squeeze()
                .float(),
                torch.as_tensor(oracle.pairwise_distances(idx, s0, np.flip(s1)))
                .squeeze()
                .float(),
            ],
            dim=0,
    ...
    # if hist and args.quantize:
    #     hist.measure(args.tag)
    if args.video:
