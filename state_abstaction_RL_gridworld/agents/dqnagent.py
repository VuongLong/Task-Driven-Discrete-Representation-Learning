...
class DQNAgent:
    ...
    def act(self, x):
        if (
            len(self.replay) < self.n_steps_init
    ...
    def get_q_predictions(self, batch):
        ...
        q_values = self.q(z)
        q_acted = extract(
            q_values, idx=torch.stack(tch(batch.a, dtype=torch.int64)), idx_dim=-1
        )
        return q_acted
    ...
    def train(self, x, a, r, xp, done):
        ...
        q_values = self.get_q_predictions(
            batch
        )  # ensure q predict pass through phi to get most recent latent loss
        ...
        if (
            self.copy_period is not None
            and self.n_training_steps % self.copy_period == 0
        ):
            self.copy_target_net()
        # self.soft_update()
